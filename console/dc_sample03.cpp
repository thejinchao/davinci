...
bool Sample03::init(void)
{
	...
	((PSMaterial*)m_ps.get())->setLightDir(fVector3(-3, -4, 10).normalise());
	((PSMaterial*)m_ps.get())->setLightColor(fVector3::WHITE);
	...
	entity->build(fMatrix4::IDENTITY, matPreviewMesh, m_vs, m_ps);
...
void Sample03::render(int32_t width, int32_t height)
{
	...
	m_model->setTransform(fMatrix4::makeScale(0.02f, 0.02f, 0.02f) * fMatrix4::makeRotate_X(-MathUtil::PI_DIV2) * fMatrix4::makeRotate_Y(m_rotateParam) * fMatrix4::makeTrans(0, -1.f, 0));
	//m_model->setTransform(fMatrix4::makeScale(2.0f, 2.0f, 2.0f));

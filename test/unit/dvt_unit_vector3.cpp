...
TEST(Math_Vector3, Basic)
{
	...
	//operator 
	{
		Vector3 v1(THREE_RANDOM_FLOAT), v2;
		v2 = v1;
		EXPECT_EQ(v1.x, v2.x);
		EXPECT_EQ(v1.y, v2.y);
		EXPECT_EQ(v1.z, v2.z);
	...
	{
		Vector3 v1(THREE_RANDOM_FLOAT), v2(v1);
		EXPECT_TRUE(v1 == v2);
		v2.x += 1.f;
		EXPECT_TRUE(v1 != v2);
	}
	{
		...
		Vector3 v1(THREE_RANDOM_FLOAT);

		Vector3 v2;
		while (v2.x == 0.f || v2.y == 0.f || v2.z == 0.f) {
			v2 = Vector3(THREE_RANDOM_FLOAT);
		}
		Vector3 v3;
	...
	{
		Vector3 v1(THREE_RANDOM_FLOAT), v2, v3;
		while (v2.x == 0.f || v2.y == 0.f || v2.z == 0.f) {
			v2 = Vector3(THREE_RANDOM_FLOAT);
		}
...
TEST(Math_Vector3, Algorithm)
{
	...
	//distance
	{
		Vector3 v1(THREE_RANDOM_FLOAT), v2(THREE_RANDOM_FLOAT);
